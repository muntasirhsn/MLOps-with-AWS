...
prefix_dataquality="DataQualityCheck" 
prefix_modelquality="ModelQualityCheck"
...
def get_pipeline(
    model_package_group_name="TestModelPackageGroup",   # Change here
    pipeline_name="TestModelPipeline",      # Change here
    base_job_prefix="TestModel",      # Change here
    ...
    model_approval_status = ParameterString(
        name="ModelApprovalStatus", default_value="Approved"  # default_value="PendingManualApproval" # model will be registered but not deployed
    )
    
    input_data = ParameterString(
        name="InputDataUrl",
        default_value=f"s3://{default_bucket}/abalone/abalone-dataset.csv",         
    )
    ...
    step_args = sklearn_processor.run(
        outputs=[
            ProcessingOutput(output_name="no_labels_train", source="/opt/ml/processing/no_labels_train"),
    ...
    data_quality_check_config = DataQualityCheckConfig(
        ...
        baseline_dataset=step_process.properties.ProcessingOutputConfig.Outputs["no_labels_train"].S3Output.S3Uri,
        dataset_format=DatasetFormat.csv(header=False, output_columns_position="START"),
        output_s3_uri=Join(on='/', values=['s3:/', default_bucket, pipeline_name, prefix_dataquality, current_time, ExecutionVariables.PIPELINE_EXECUTION_ID, 'dataqualitycheckstep'])
    )
...
## --- Training Step --- ##
    ...
    model_path = f"s3://{default_bucket}/{pipeline_name}/ModelTrain/{current_time}"        
    ...
    xgb_train = Estimator(
        ...
        base_job_name=f"{pipeline_name}/modeltrain",       
        sagemaker_session=pipeline_session,
        role=role,
    )
    ...
    transformer = Transformer(
        ...
        accept="text/csv",
        assemble_with="Line",
        output_path=f"s3://{default_bucket}/{pipeline_name}/ModelTransform/{current_time}",     
        sagemaker_session=pipeline_session,
    )
    ...
    model_quality_check_config = ModelQualityCheckConfig(
        ...
        output_s3_uri=Join(on='/', values=['s3:/', default_bucket, pipeline_name, prefix_modelquality, current_time, ExecutionVariables.PIPELINE_EXECUTION_ID, 'modelqualitycheckstep']),
    ...
    script_eval = ScriptProcessor(
        ...
        instance_type=processing_instance_type,
        instance_count=1,
        base_job_name=f"{base_job_prefix}/ModelEval",                      
        sagemaker_session=pipeline_session,
        role=role,
    )
    ...
    step_args = model.register(
        ...
        customer_metadata_properties={"ModelName": step_create_model.properties.ModelName},
        model_package_group_name=model_package_group_name,
    ...
    endpoint_config_name = "trial-model-endpoint-config"
    endpoint_name = "trial-model-endpoint-" + current_time
    ...
    step_deploy_model_lambda = LambdaStep(
        ...
        inputs={
            ...
            "endpoint_instance_type": endpoint_instance_type # "ml.m5.xlarge" 
        },
    )
    ...
    step_cond = ConditionStep(
        name="Check-Model-Evaluation-Metric",
        conditions=[cond_lte],
        if_steps=[
            step_create_model,   # execute  'step_create_model' if step_cond is satisfied
            step_register,               # execute  'step_register' if step_cond is satisfied
    ...
    pipeline = Pipeline(
        ...
        steps=[step_process, data_quality_check_step, step_train, step_transform, model_quality_check_step, step_eval, step_cond], 
        sagemaker_session=pipeline_session
    )
...
print("Model Build Pipeline complete.")